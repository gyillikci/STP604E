...
mmm=2;            % number of variables 
nnn=10;          % population size 
...
for run=1:maxrun     
    ...
    for iii=1:nnn   
%         iii
        f0(iii,1)=pso_mishrasbird_function(x0(iii,:));     
    end
    [fmin0,index0]=min(f0);         
    ...
    while ite<=maxite && tolerance>10^-12
         ...
         % evaluating fitness         
         for iii=1:nnn
             f(iii,1)=pso_mishrasbird_function(x(iii,:));
         end
