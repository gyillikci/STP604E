...
Uses absolute paths to ensure images are found correctly
"""
...
ASSIGNMENT_DIR = SCRIPT_DIR / "assignment_2"
...
def image_to_base64(image_name):
    ...
    image_path = ASSIGNMENT_DIR / image_name
    if not image_path.exists():
        print(f"  WARNING: Image not found: {image_path}")
        return None
    with open(image_path, 'rb') as f:
        data = base64.b64encode(f.read()).decode('utf-8')
    print(f"  Embedded: {image_name} ({image_path.stat().st_size/1024:.1f} KB)")
...
CSS_STYLES = """
    @media print { 
        ...
        .graph-container { page-break-inside: avoid; margin: 20px 0; }
    }
    ...
    .graph-container img { max-width: 100%; height: auto; border: 1px solid #ddd; border-radius: 5px; box-shadow: 0 2px 8px rgba(0,0,0,0.15); }
    .img-caption { text-align: center; font-style: italic; color: #666; margin-top: 10px; font-size: 13px; }
"""
...
def generate_problem1_html():
    print("\n  Embedding images for Problem 1...")
    img_convergence = image_to_base64("problem1_convergence.png")
    img_cte = image_to_base64("problem1_cte_variation.png")
    
    convergence_html = ""
    if img_convergence:
        convergence_html = f'''
<div class="graph-container">
    <img src="{img_convergence}" alt="PSO Convergence" style="width:100%; max-width:800px;">
    <p class="img-caption">Figure 1: PSO Convergence and Solution Space Exploration</p>
</div>'''
    ...
    if img_cte:
        cte_html = f'''
<div class="graph-container">
    <img src="{img_cte}" alt="CTE Variation" style="width:100%; max-width:800px;">
    <p class="img-caption">Figure 2: Thermal Expansion Coefficients vs. Fiber Angles</p>
</div>'''
...
<div class="header-info">
    <p><strong>Course:</strong> STP604E - Advanced Design, Analysis and Optimization of Composite Structure for Aerospace</p>
...
def generate_problem2_html():
    print("\n  Embedding images for Problem 2...")
    img_results = image_to_base64("problem2_results.png")
    img_params = image_to_base64("problem2_parameter_study.png")
    
    results_html = ""
    if img_results:
        results_html = f'''
<div class="graph-container">
    <img src="{img_results}" alt="Problem 2 Results" style="width:100%; max-width:800px;">
    <p class="img-caption">Figure 1: Optimization Results - Convergence, Ply Distribution, and Property Comparison</p>
</div>'''
    ...
    if img_params:
        params_html = f'''
<div class="graph-container">
    <img src="{img_params}" alt="Parameter Study" style="width:100%; max-width:800px;">
    <p class="img-caption">Figure 2: Parameter Sensitivity Study - Population Size and Iteration Effects</p>
</div>'''
...
def generate_problem3_html():
    print("\n  Embedding images for Problem 3...")
    img_continuous = image_to_base64("problem3_continuous_results.png")
    img_comparison = image_to_base64("problem3_comparison.png")
    
    continuous_html = ""
    if img_continuous:
        continuous_html = f'''
<div class="graph-container">
    <img src="{img_continuous}" alt="Continuous Results" style="width:100%; max-width:800px;">
    <p class="img-caption">Figure 1: Lamination Parameter Feasible Region and PSO Convergence</p>
</div>'''
    ...
    if img_comparison:
        comparison_html = f'''
<div class="graph-container">
    <img src="{img_comparison}" alt="Comparison" style="width:100%; max-width:800px;">
    <p class="img-caption">Figure 2: Comparison of Three Design Cases</p>
</div>'''
...
def generate_problem4_html():
    print("\n  Problem 4 has no images (mathematical derivation)")
    return f"""<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
def main():
    ...
    print("="*60)
    print("Generating Assignment 2 PDFs with Embedded Graphs")
    print("="*60)
    ...
    print(f"✓ Assignment folder: {ASSIGNMENT_DIR}")
    ...
    for num, generator in problems:
        ...
        html_file = ASSIGNMENT_DIR / f"PROBLEM{num}_ANALYSIS.html"
        pdf_file = ASSIGNMENT_DIR / f"PROBLEM{num}_ANALYSIS.pdf"
        ...
        print(f"  ✓ HTML: {html_file.name} ({html_size:.1f} KB)")
        
        # Convert to PDF
        if html_to_pdf(html_file, pdf_file, browser):
            size = pdf_file.stat().st_size / 1024
            print(f"  ✓ PDF: {pdf_file.name} ({size:.1f} KB)")
    ...
    # Delete old file if exists to avoid permission errors
    output_file = ASSIGNMENT_DIR / "ASSIGNMENT2_FINAL_REPORT.pdf"
    if output_file.exists():
        try:
            output_file.unlink()
        except PermissionError:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            output_file = ASSIGNMENT_DIR / f"ASSIGNMENT2_FINAL_REPORT_{timestamp}.pdf"
            print(f"⚠ Original file locked, creating: {output_file.name}")
    ...
    print(f"\n{'='*60}")
    print(f"✓ Final PDF: {output_file.name}")
